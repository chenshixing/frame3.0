...
(function ($) {
    ...
    var pluginList = {
        ...
        pdf: {
            ...
            plugin: /adobe\s?acrobat|Chrome PDF Viewer/gim
        },
        qtime: {
    ...
    var isSupported = function (p) {
        ...
        if (window.ActiveXObject || /MSIE|Trident/g.test(UA)) {
            $.browser[p] = false;

            for (var i = 0; i < pluginList[p].activex.length; i++) {
                try {
...
//
    function generate(el, opts, player) {
        ...
        if (player == 'iframe') {
            ...
            o.attr('src', opts.src);
            o.css({'backgroundColor': o.bgColor, 'overflow': 'hidden'}).attr({scrolling : "no"});
        }
        else if (player == 'img') {
            o = $('<img>');
            o.attr('src', opts.src);
