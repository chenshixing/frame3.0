            ...
            uploader.on('uploadSuccess', function (file, data) {
                ...
                if (data.status == '1') {
                    ...
                    if (!p.multiple) {
                        ...
                        parentObj.siblings(".uploadName").val(data.name);
                        parentObj.siblings(".upload-name").text(name + "." + names[1]);
                        parentObj.siblings(".upload-path").val(data.path);
                        parentObj.siblings(".upload-size").val(data.size);

                        // 显示上传成功的图片
                        var $parents =  parentObj.parents('.form-group');
                        $parents.find('.upload-img-yours').attr('src', file.name);
                        // 图片验证不通过
                        if(data.info == '0') {
                            $parents.find('.upload-img-alert').show();
                        }
                    } else {
